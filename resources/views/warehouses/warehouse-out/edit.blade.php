@extends('layouts.app')
@push('title')
    <title>WAREHOUSE OUT UPDATE</title>
@endpush
@section('index')
    WAREHOUSE OUT
@endsection
@section('content')
    <form action="{{ route('warehouse-out.update', $warehouse_out->id) }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-header">
                WAREHOUSE OUT UPDATE
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <select name="batch_no" id="batch_id" class="form-select">
                                ...
                                @foreach ($batches as $batch)
                                    <option value="{{ $batch->id }}" @selected($warehouse_out->batch_id == $batch->id)>{{ $batch->batch_no }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <textarea name="remarks" class="form-control" id="" cols="30" rows="1">{{ $warehouse_out->remarks }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <input name="date" type="date" value="{{ $warehouse_out->date }}" class="form-control">
                        </div>
                    </div>
